...
chrome.runtime.onInstalled.addListener((details) => {
  console.log('[DEBUG] TaskMaster后台服务已安装/更新', details);
  initAutoBackup();
  checkCoreFiles();

  // 如果是版本升级，自动触发一次数据备份并打开设置页提醒用户
  if (details.reason === 'update' && details.previousVersion) {
    console.log(`[DEBUG] 检测到扩展从 ${details.previousVersion} 升级到 3.0.0`);
    
    // 打开设置页面并附带 upgrade 参数，通知用户进行手动导出
    chrome.tabs.create({ url: 'options.html?upgrade=true' });
